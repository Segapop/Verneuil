...
public record GemDataRecord(UUID uuid, float healthModifier) {
	public static final Codec<GemDataRecord> BASIC_CODEC = RecordCodecBuilder.create(instance -> instance.group(UUIDUtil.CODEC.fieldOf("gem_uuid").forGetter(GemDataRecord::uuid), Codec.FLOAT.fieldOf("health_modifier").forGetter(GemDataRecord::healthModifier)).apply(instance, GemDataRecord::new));
